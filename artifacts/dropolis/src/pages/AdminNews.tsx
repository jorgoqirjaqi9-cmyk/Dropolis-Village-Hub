...
type NewsSubmission = {
  id: number; title: string; content: string; villageId: number | null;
  senderName: string; senderEmail: string | null; eventDate: string;
  eventTime: string | null; imageUrl: string | null;
  status: string; consentGiven: boolean; submittedAt: string; reviewedAt: string | null;
};
...
export default function AdminNews() {
  ...
  const fetchSubmissions = useCallback(async (t: string, status: StatusFilter) => {
    ...
    try {
      const res = await adminFetch(`/api/news-submissions?status=${status}`, t);
  ...
  async function approve(id: number) {
    setActionLoading(id);
    try {
      const res = await adminFetch(`/api/news-submissions/${id}`, token, {
        method: "PATCH", body: JSON.stringify({ status: "approved" }),
      });
      if (!res.ok) throw new Error();
      setSubmissions(prev => prev.filter(s => s.id !== id));
      showMsg("✓ Εγκρίθηκε και δημοσιεύτηκε ως άρθρο.");
  ...
  async function deleteSub(id: number) {
    setActionLoading(id);
    try {
      const res = await adminFetch(`/api/news-submissions/${id}`, token, { method: "DELETE" });
  ...
  const statusLabel: Record<StatusFilter, string> = { pending: "Αναμονή", approved: "Εγκεκριμένες", rejected: "Απορριφθείσες" };

  return (
    <>
      <SEO title="Υποβολές Ειδήσεων — Admin" noindex />
      <AdminLayout token={token} onLogout={logout} title="Διαχείριση Υποβολών Ειδήσεων">
        <div className="max-w-5xl mx-auto space-y-4">
          ...
          {!loading && submissions.length === 0 && (
            <div className="text-center py-16 text-muted-foreground text-sm">Δεν υπάρχουν υποβολές σε αυτή την κατηγορία.</div>
          )}
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1.5 text-xs text-muted-foreground">
                        ...
                        <span className="flex items-center gap-1"><User className="w-3 h-3" />{sub.senderName}{sub.senderEmail && ` <${sub.senderEmail}>`}</span>
                        {sub.villageId && <span className="flex items-center gap-1"><MapPin className="w-3 h-3" />Χωριό #{sub.villageId}</span>}
                        <span className="flex items-center gap-1"><Calendar className="w-3 h-3" />{sub.eventDate}{sub.eventTime && <><Clock className="w-3 h-3 ml-1" />{sub.eventTime}</>}</span>
                        {sub.imageUrl && (
        ...
        {deleteId && (
          <ConfirmModal
            message="Διαγραφή αυτής της υποβολής; Η ενέργεια δεν αναιρείται."
